...
use super::errors::NetworkTypeError;
...
///
/// DNS name resolution for [`IpOrHost::Dns`] instances is provided by runtime-specific
/// helpers in the `hopr-utilities` crate (e.g. `resolve_tokio`).
...
///
/// // On the Client
/// let encrypted = SealedHost::seal(host.clone(), exit_node_peer_id)?;
///
/// // On the Exit node
...
impl SealedHost {
    ...
    pub fn seal(host: IpOrHost, peer_id: PeerId) -> super::errors::Result<Self> {
        ...
        if host_str.len() < Self::MAX_LEN_WITH_PADDING {
            let pad_len =
                crate::crypto_random::random_integer(0, (Self::MAX_LEN_WITH_PADDING as u64).into());
            for _ in 0..pad_len {
                host_str.push(Self::PADDING_CHAR);
            }
        }

        crate::crypto::seal::seal_data(host_str.as_bytes(), peer_id)
            .map(Self::Sealed)
    ...
    pub fn unseal(
        self,
        key: &crate::crypto::keypairs::OffchainKeypair,
    ) -> super::errors::Result<IpOrHost> {
        match self {
            SealedHost::Plain(host) => Ok(host),
            SealedHost::Sealed(enc) => crate::crypto::seal::unseal_data(&enc, key)
                .map_err(|e| NetworkTypeError::Other(e.to_string()))
                .and_then(|data| {
...
#[cfg(test)]
mod tests {
    use crate::crypto::prelude::{Keypair, OffchainKeypair};
