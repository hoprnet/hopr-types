...
pub(crate) fn random_group_element() -> ([u8; 32], NonIdentity<AffinePoint>) {
    ...
    // let scalar = k256::NonZeroScalar::random(&mut crate::crypto_random::rng());

    // Requires manual rejection sampling implementation due to incompatible rand crates
    let mut rng = crate::crypto_random::rng();
    let mut bytes = k256::FieldBytes::default();
    use crate::crypto_random::Rng;
    loop {
...
pub fn x25519_scalar_from_bytes(
    bytes: &[u8],
) -> crate::crypto::errors::Result<curve25519_dalek::scalar::Scalar> {
    if bytes.len() == 32 {
...
pub fn sample_secp256k1_field_element(
    secret: &[u8],
    tag: &str,
